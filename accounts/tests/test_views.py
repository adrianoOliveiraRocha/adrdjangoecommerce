...
class RegisterViewTestCase(TestCase):
    ...
    def test_register_error(self):
        data = {'username': 'adriano', 'password1': 'teste123',
                'password2': 'teste123'}
        response = self.client.post(self.register_url, data)
...
class UpdateUserTestCase(TestCase):
    ...
    def test_update_user_ok(self):
        ...
        self.assertRedirects(response, accounts_index_url) # test if went redirect
#        user = User.objects.get(username=self.user.username). optional way for get actual user
        self.user.refresh_from_db() # get the actual user

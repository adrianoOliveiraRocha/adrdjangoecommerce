...
class CartItemManage(models.Manager):
    def add_item(self, cart_key, product):
        # cart_item captures the item created or redeemed
        # created is a boolean that indicates whether the item was created
...
class CartItem(models.Model):
    ...
    objects = CartItemManage()

    class Meta:
        verbose_name = 'Item do Carrinho'
        verbose_name_plural = 'Itens dos Carrinhos'
        # Prevents the same product from being placed twice in the cart
...
class Order(models.Model):
    ...
    payment_option = models.CharField('Opção de Pagamento',
                                      choices=PAYMENT_OPTIONS_CHOICES,
                                      max_length=20,
                                      default='deposit')
...
#        Take the id of each product
    ...
    def paypal(self):
        ...
        index = 1
        for item in self.items.all():
            paypal_dict['amount_{}'.format(index)] = '%2.f' % item.price

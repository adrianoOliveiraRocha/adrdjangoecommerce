...
class CreateCartItemView(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        ...
        # If the user has not yet used the session, it needs to be created
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if session_key and CartItem.objects.filter(cart_key=session_key).exists():
            ...
            messages.info(request, 'Não há ítens no carrinho de compras')
...
class OrderDetailView(LoginRequiredMixin, DetailView):
    """Only one order is returned based on the request pk that is passed in the
     url. However, in order for a user to not be able to detail an order that is not
     the get_queryset method gives a list of possible orders for the
     user"""
